...
class PasswordResetService
{
    ...
    public function resetPassword(Request $request, array $credentials): void
    {
        ...
        if ($user === null) {
            $this->logResetFailure($request, null, $normalizedEmail, 'unknown_email');
        ...
        if (! $this->broker()->tokenExists($user, $token)) {
            $this->logResetFailure($request, $user, $normalizedEmail, $this->resolveInvalidTokenReason($normalizedEmail));
            $this->throwInvalidResetLink();
        }

        if (Hash::check($password, (string) $user->password)) {
            $this->logResetFailure($request, $user, $normalizedEmail, 'password_reuse');
    ...
    protected function resolveInvalidTokenReason(string $normalizedEmail): string
    {
        ...
        try {
            $record = DB::table($table)->where('email', $normalizedEmail)->first();
        } catch (\Throwable) {
