/**
 * Shared Google Forms–style question builder (GForm Builder module)
 * Used by Scholarship, Sports, Survey, and other program forms.
 */
(function (global) {
    const INPUT_CLASS = 'gform-input sports-input schol-input';
    let spfbQuestions = [];
            ...
            modal.innerHTML = `
                <div class="spfb-delete-confirm-box">
                    ...
                    <div class="spfb-delete-confirm-actions">
                        <button type="button" class="gform-btn gform-btn-outline sports-btn sports-btn-outline" id="spfbDeleteCancelBtn">Cancel</button>
                        <button type="button" class="gform-btn gform-btn-danger sports-btn sports-btn-danger" id="spfbDeleteConfirmBtn">Yes, Delete</button>
                    </div>
                </div>
            `;
    ...
    function buildPreviewCard(q, idx) {
        ...
        if (q.type === 'text') {
            ...
            previewHTML = `<div class="spfb-preview-options">${opts.map(o =>
                `<label class="spfb-preview-option"><input type="${inputType}" disabled> <span>${escapeHtml(o || '(empty)')}</span></label>`
            ).join('')}</div>`;
        ...
        const labelDisplay = q.label
            ? escapeHtml(q.label)
            : `<em style="color:#9ca3af;">Untitled Question ${idx + 1}</em>`;
    ...
    function buildEditCard(q, idx) {
        ...
        <div class="spfb-options-section" id="spfbOptionsSection_${q.id}">
            ...
            <div class="spfb-options-list" id="spfbOptionsList_${q.id}">
                ...
                <div class="spfb-option-row">
                    ...
                    <input type="text" class="spfb-option-input ${INPUT_CLASS}" value="${escapeHtml(o)}" placeholder="Option ${oi + 1}" maxlength="200">
        ...
        <div class="spfb-edit-body">
            ...
            <div class="spfb-edit-field">
                ...
                <input type="text" class="spfb-q-label-input ${INPUT_CLASS}" value="${escapeHtml(q.label)}" placeholder="Enter your question here..." maxlength="300">
            </div>

            <!-- Question Type -->
            <div class="spfb-edit-field">
                <label class="spfb-edit-field-label">Input Type</label>
                <select class="spfb-q-type-select ${INPUT_CLASS}" style="cursor:pointer;">
                    ${typeOptions}
                </select>
            </div>
    ...
    global.GFormBuilder = api;
    global.SpfbFormBuilder = api;
})(window);
