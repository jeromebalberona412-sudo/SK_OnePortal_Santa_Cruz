...
function pickerHtml(active) {
    return `<div class="cp-picker"><div class="cp-picker-inner">${
        Object.entries(REACTION_EMOJI).map(([type, emoji]) =>
            `<button type="button" class="cp-picker-opt" data-type="${type}" title="${type}">${emoji}</button>`
        ).join('')
    }</div></div>`;
}
...
function commentHtml(comment, isReply) {
    ...
    const type = comment.reaction_type || '';
    const badge = likes > 0
...
async function submitComment(body, parentId = null) {
    ...
    try {
        ...
        const comment = await apiFetch(`/api/community-feed/${post.id}/comments`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
