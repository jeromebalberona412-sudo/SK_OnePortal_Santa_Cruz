...
class EmailChangeVerificationNotification extends Notification
{
    ...
    {
        $url = url(route('profile.change-email.confirm', [
            'id' => $this->user->id,
            'token' => $this->plainToken,
        ], false));

        return (new MailMessage)
            ->subject('Confirm Your New OnePortal Admin Email')
            ->greeting('Hello!')
            ->line('You requested to change the email address on your OnePortal Admin account.')
